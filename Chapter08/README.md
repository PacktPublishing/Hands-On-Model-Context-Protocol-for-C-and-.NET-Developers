...
`d:/gitbook/ModelContextProtocol/chapters/Ch08.md` for full explanations.
...
- Reference snippet files (`ch08_*.cs`) excluded from compilation
...
snippets. The numbered `ch08_*.cs` files are the verbatim chapter
listings and are not built; they exist as reading material.

## Code samples

See the `code/` directory. Files are named `ch08_N_description.cs`.
...
When a Chapter 8 snippet references a service such as
