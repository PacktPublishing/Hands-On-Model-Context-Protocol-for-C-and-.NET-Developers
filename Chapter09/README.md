...
snippets. The numbered `ch09_*.cs` files are the verbatim chapter
listings and are not built; they exist as reading material.

## Code samples

See the `code/` directory. Files are named `ch09_N_description.cs`.
...
When a Chapter 9 snippet references a service such as
