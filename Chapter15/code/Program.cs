...
//
// Runs the ch15_*.cs adaptations from Demos.cs:
//   1. TokenUsageTracker aggregates prompt/completion/cached tokens (ch15_6).
//   2. BudgetCapEnforcer blocks a workflow once its token cap is exceeded (ch15_7).
//   3. LlmResponseKeyBuilder shows normalised prompts sharing a cache key and
//      scope-based TTLs (ch15_8).
//   4. ConsistentHashRouter distributes keys across a hash ring and stays
//      stable when a server is removed (ch15_9).
//   5. LoadShedder classifies requests by priority and sheds low-priority
//      traffic under load while never shedding critical calls (ch15_10).

using TravelBooking.Chapter15;

Console.WriteLine("Chapter 15 -- Observe and Scale MCP: Metrics, Tracing, Costs, and Sharding");
Console.WriteLine(new string('=', 78));

Console.WriteLine();
Console.WriteLine("[1] TokenUsageTracker -- ch15_6");
...
Console.WriteLine();
Console.WriteLine("[2] BudgetCapEnforcer -- ch15_7");
...
Console.WriteLine();
Console.WriteLine("[3] LlmResponseKeyBuilder -- ch15_8");
...
Console.WriteLine();
Console.WriteLine("[4] ConsistentHashRouter -- ch15_9");
...
Console.WriteLine();
Console.WriteLine("[5] LoadShedder -- ch15_10");
...
Console.WriteLine();
Console.WriteLine("Chapter 15 demo complete.");
